...
export class CrmAdapterFactory {
  ...
  static async getAdapter(tenantId: string): Promise<Result<ICrmAdapter, AppError>> {
    ...
    if (!settings) {
      logger.warn(`[CRM] Tenant settings not found for tenant ${tenantId}`);
    ...
    if (settings.staysnetClientSecret) {
      try {
        ...
        logger.info(`[CRM] StaysNet adapter initialized for tenant ${tenantId} | domain: ${domain} | token length: ${base64Token.length}`);
        const credentials = { base64Token, domain };
        return ok(new StaysNetAdapter(credentials));
      } catch (err: unknown) {
        logger.error({ err }, `[CRM] Failed to decrypt Stays.net credentials for tenant ${tenantId}`);
        return fail(new AppError("Failed to decrypt Stays.net credentials"));
      }
    }

    // Default fallback if no CRM is configured
    logger.warn(`[CRM] No CRM integration configured for tenant ${tenantId} (staysnetClientSecret is empty)`);
